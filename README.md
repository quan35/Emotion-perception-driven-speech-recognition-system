...
flowchart LR
  subgraph Train["训练阶段"]
    D["数据集<br/>RAVDESS / CASIA / TESS / ESD"]
    P["预处理<br/>降噪 + 静音切除 + 重采样 + 定长<br/>preprocessing/audio_preprocess.py"]
    F["特征提取<br/>Mel(128) + MFCC<br/>preprocessing/feature_extract.py"]
    T["训练 SER 模型<br/>notebooks/03_train_emotion.ipynb<br/>notebooks/04_train_shared.ipynb"]
    C["保存模型权重<br/>checkpoints/best_emotion.pth<br/>checkpoints/best_shared.pth"]
    D --> P --> F --> T --> C
  end

  subgraph Use["使用阶段（UI 界面）"]
    U["用户录音 / 上传音频文件<br/>ui/app.py"]
    A["ASR: Whisper 转写<br/>inference/pipeline.py::ASREngine"]
    S["SER: 情感识别<br/>CNN+BiLSTM+Attention 或 Whisper 共享编码器<br/>inference/pipeline.py::EmotionAwareSpeechPipeline"]
    V["可视化展示<br/>文本着色 + 雷达图 + 波形/频谱 + 历史趋势<br/>utils/visualization.py"]
    U --> A --> S --> V
  end
...
若你的 Markdown 渲染环境不支持 Mermaid，可按下面的流程理解：
...
- **使用阶段（UI）**
  ...
  - **SER**：两种情感模型可切换（`inference/pipeline.py`）
...
- **命令行运行时**：在启动命令前显式设置（推荐）。
  ...
  ```
- **Notebook 场景**：Jupyter kernel 进程可能不继承当前 shell 的环境变量，建议在 kernel 启动环境中设置（如 kernelspec 的 `env`），并重启 kernel。

### 2) `soundfile.LibsndfileError: Error opening ... : System error`

**现象**：预处理 ESD 等大数据集时，写入 `data/processed/.../*.wav` 失败。

**常见原因**：

- 磁盘空间不足（ESD 全量 + 定长输出会明显增大体积）。
- inode 用尽（小文件数量巨大时可能发生）。
...
### 3) `04_train_shared.ipynb` 的“准备数据集”阶段内存/资源占用高
...
- 用 `DataLoader(num_workers>0)` 并行读取音频（适合 16vCPU），并在 GPU 端批量运行 encoder。
...
### 4) GPU 利用率高但显存占用不高（提特征阶段）

**解释**：前向推理 + `no_grad` + 混合精度会显著降低显存需求；同时工作负载可能是 compute-bound，因此出现“GPU 很忙但显存占用不高”。这属于正常现象。

### 5) 训练阶段 GPU 利用率低但训练很快（共享编码器方案）

**解释**：共享编码器方案在训练阶段只训练轻量 MLP 分类头；encoder 重计算已在“准备数据集/缓存特征”完成，因此训练阶段对 GPU 需求很小，速度快是预期行为。

### 6) 常见 notebook 运行错误

- **`NameError: whisper_size is not defined`**：建议在“构建模型”阶段直接从 `configs/config.yaml` 读取 `whisper_size`，避免依赖前序 cell 的临时变量。
- **memmap 只读警告**（non-writable numpy）：当特征通过 `mmap_mode='r'` 读取时，切片可能不可写。建议在 Dataset 中复制为可写数组再转 tensor（项目已处理）。
