...
class WhisperEmotionFinetune(nn.Module):
    """
    可选: 解冻 Whisper Encoder 最后几层进行微调的版本。
    适合数据量较大时使用，效果通常优于纯冻结方案。
    """

    def __init__(self, whisper_model, num_classes=6, unfreeze_last_n=2):
        super().__init__()
        self.encoder = whisper_model.encoder
        enc_dim = self.encoder.ln_post.normalized_shape[0]
